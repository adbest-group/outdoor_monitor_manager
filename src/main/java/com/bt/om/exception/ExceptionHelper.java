 ...
 * 
 * @author tany</a>
 * @version create on 2011-5-18 下午03:05:46
 */
public class ExceptionHelper {
    ...
    private static Throwable getThrowable(ChainedThrowable throwable) {
        if (ChainedThrowableDelegate.class.isInstance(throwable)) {
    ...
    public static ChainedThrowable getChainedThrowable(Throwable throwable) {
        if ((throwable != null) && !(ChainedThrowable.class.isInstance(throwable))) {
    ...
    private static void printThrowableMessage(ChainedThrowable throwable, PrintWriter writer,
        ...
        if (cause) {
            builder.append(STRING_CAUSED_BY);
        }
        ...
        if (message != null) {
            builder.append(STRING_EXCEPTION_MESSAGE).append(message);
        }
    ...
    private static class StackTraceAnalyzer {
        ...
        private class StackTraceEntry implements Comparable<Object> {
            private List<String> lines             = new ArrayList<String>(10);

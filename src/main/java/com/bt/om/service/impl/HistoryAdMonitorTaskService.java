 ...
 */
@Service
public class HistoryAdMonitorTaskService implements IHistoryAdMonitorTaskService {
    @Autowired
	...
	@Autowired
	private HistoryAdMonitorTaskFeedbackMapper historyAdMonitorTaskFeedbackMapper;
    ...
    public void getPageData(SearchDataVo vo) {
        int count = historyAdMonitorTaskMapper.getPageCount(vo.getSearchMap());
        vo.setCount(count);
        if (count > 0) {
            vo.setList(historyAdMonitorTaskMapper.getPageData(vo.getSearchMap(), new RowBounds(vo.getStart(), vo.getSize())));
        } else {
...
//            feedback.setMonitorTaskId(taskId);
     ...
     */
    @Override
    public AdMonitorTaskVo getTaskDetails(SearchDataVo vo) {
//        int taskIds = Integer.valueOf(taskId);
        return historyAdMonitorTaskMapper.getTaskDetails(vo.getSearchMap());
    }
     ...
     */
    @Override
    public List<AdMonitorTaskVo> getSubmitDetails(SearchDataVo vo) {
//        int taskIds = Integer.valueOf(taskId);
        List<AdMonitorTaskVo> list = historyAdMonitorTaskMapper.getSubmitDetails(vo.getSearchMap());
        return list;
    }
	 ...
	 */
	@Override
	public List<AdMonitorTaskFeedback> selectByActivity(String tableName,List<Integer> monitorTaskIds) {
		return historyAdMonitorTaskFeedbackMapper.selectByActivity(tableName,monitorTaskIds);
	}
	...
	public void newActivateMonitorTask(Date nowDate) {
		...
		if(taskAdSeats != null && taskAdSeats.size() > 0) {
			...
			for (TaskAdSeat taskAdSeat : taskAdSeats) {
		        ...
		        userIds = sysUserMapper.getUserId(6);//6:呼叫中心人员
		        userIdList.addAll(userIds);
		        userIdList.addAll(cuslist);
	 ...
	 */
	@Override
	public List<AdMonitorTask> getAllTasksByActivityId(SearchDataVo vo) {
		return historyAdMonitorTaskMapper.getAllTasksByActivityId(vo.getSearchMap());
	}
	...
	public List<AdMonitorTask> newSelectLatestMonitorTaskIds(Map<String, Object> searchMap) {
		return historyAdMonitorTaskMapper.newSelectLatestMonitorTaskIds(searchMap);
	}

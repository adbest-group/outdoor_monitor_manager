 ...
 */
@Service
public class HistoryPDFHelper {
    private static final Logger logger = Logger.getLogger(HistoryPDFHelper.class);
...
//            table.addCell(new Paragraph(list.get(8), fontChinese));//结束监测时间
    ...
    private void insertLogo(Document document, AdApp adapp) throws IOException, DocumentException {
        ...
        //插入广告logo
        if(adLogo==null) {
            adLogo = Image.getInstance(adapp.getAppPictureUrl().indexOf("http")==-1?real_path:"" + adapp.getAppPictureUrl());

...
(function(){
    ...
    MarkerClusterer.prototype.addMarkers = function(markers){
        for(var i = 0, len = markers.length; i <len ; i++){
			this._markers.push(markers[i]);//Marker拖放后enableDragging不做变化，忽略
			markers[i].isInCluster = false;
        }
     ...
     * 根据所给定的标记，创建聚合点
     * @return 无返回值
	 * 黄透平修改于2016年5月20日。主要修改内容：
	 * 1、所有点位聚合完成后再刷新所有的图标，避免每聚合一个点位刷新一次图标，明显加快速度。
	 * 2、计算某点与所有聚合点中心距离时，先计算是否在该聚合范围内，以减少求两点距离的次数。
	 * 3、求两点距离的公式运算较复杂，费时多，此处仅需用勾股定理求到距离的平方后找最近的点即可，故换了个公式，单点运算时间仅为原来的十分之一。
     */
    MarkerClusterer.prototype._createClusters = function(){
		...
		for(var i = 0, marker; marker = this._markers[i]; i++){
			if(!marker.isInCluster && extendedBounds.containsPoint(marker.getPosition()) ){ 
				...
				for(var t = 0, cluster; cluster = this._clusters[t]; t++){
					...
					if(center&& cluster.isMarkerInClusterBounds(marker)){
						var d = Math.pow(2,(center.lat-marker.getPosition().lat))+Math.pow(2,(center.lng-marker.getPosition().lng));
						if(d < distance){
							distance = d;
				...
				if (clusterToAddTo){
					...
					clusterToAddTo._markers.push(marker);
				} else {
		...
		{            
			var len = cluster._markers.length;
			if(len < cluster._minClusterSize )
			{
				for (var t = 0; t < cluster._minClusterSize; t++) 
				{
					this._map.addOverlay(cluster._markers[t]);						
				}
			}
			else
			{
